...
function TeamsScreen({ theme }: { theme: any }) {
  ...
  const [userOrgs, setUserOrgs] = useState<{orgId: string; orgName: string; role: string}[]>([]);
  ...
  const loadTeamsForUserOrgs = async () => {
    try {
      ...
      for (const org of userOrgs) {
        const qTeams = query(collection(db, "teams"), where("orgId", "==", org.orgId));
  ...
  const handleAddMember = async () => {
    ...
    try {
      ...
      const q = query(collection(db, "users"), where("email", "==", newMemberEmail.trim().toLowerCase()));
      ...
      <Portal>
        ...
        >
          ...
          <Dialog.Content>
            ...
            <Subheading style={{ marginTop: 8 }}>Choose Organization:</Subheading>
            {userOrgs
          ...
          <Dialog.Content>
            {targetTeam && (
              <>
                ...
                <RadioButton.Group
                  onValueChange={(val) => setNewMemberRole(val as "member" | "manager")}
                  value={newMemberRole}
                >
